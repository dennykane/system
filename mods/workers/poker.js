...
//const SLEEP_EVERY_NUM_HANDS = 5000;
let SLEEP_EVERY_NUM_HANDS = 1000;
let TEXT_ONLY = false;
...
const deal_cards = async() => {//«

if (round_num === RIVER_ROUND) return await showdown();
...
if (round_num === FLOP_ROUND){
	flop_suit_types = [0,0,0,0];
	suit_types = flop_suit_types;
}
else if (round_num === TURN_ROUND){
	turn_suit_types = flop_suit_types.slice();
	suit_types = turn_suit_types;
}
else if (round_num === RIVER_ROUND){
	river_suit_types = turn_suit_types.slice();
	suit_types = river_suit_types;
}
...
onmessage=(e)=>{//«
	let s = e.data;
	if (s==="POLL"){
		self.postMessage({FLOPS, TURNS, RIVERS});
//		self.postMessage({HOLES, FLOPS, TURNS, RIVERS});
//		HOLES={};
		FLOPS={};
		TURNS={};
		RIVERS={};
